...
export async function getStockByNames(symbols) {
  console.log("call getStockByNames");
  ...
  // allStocks = [ {symbol: 'AMC', currentPrice: 45, afterHoursPrice: ..., ...}, { symbol: 'AAPL' , ...} ]
...
export async function getMovers(start, pageLimit = STOCKS_PER_PAGE) {
  console.log("call getMovers");

  var url = new URL("http://localhost:3000/api/movers");
  var params = { start, count: pageLimit };
  url.search = new URLSearchParams(params).toString();
  ...
  // movers = { dayGainers: {total: 200, data: ['AMC', 'AAPL' ... ]},
  //            dayLosers:  {total: 10, data: [ ... ]},
  //            mostActives: {total: 120, data: ['AMC', 'AAPL' ... ]} }
...
export function setFloatingPoint(num) {
  if (!num) return;
  return num.toFixed(FLOATING_POINT);
}
...
export async function getRealTimePrice(symbols) {
  var options = {
    ...
    params: { symbol: symbols.toUpperCase(), format: "json", outputsize: "30" },
    headers: {
      "x-rapidapi-key": process.env.NEXT_PUBLIC_API_SECRET_KEY,
      "x-rapidapi-host": "twelve-data1.p.rapidapi.com",
    },
